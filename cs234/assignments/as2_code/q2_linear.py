...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, shape=(None, height, width, channel * self.config.state_history))
        self.a = tf.placeholder(tf.int32, shape=(None, ))
        self.r = tf.placeholder(tf.float32, shape=(None, ))
        self.sp = tf.placeholder(tf.uint8, shape=(None, height, width, channel * self.config.state_history))
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_collect = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)
        target_q_collect = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        op = [
            tf.assign(target_q_collect[i], q_collect[i])
            for i in range(len(q_collect))
        ]
    ...
    def add_loss_op(self, q, target_q):
        ...
        mask = 1 - tf.cast(self.done_mask, dtype=tf.float32)
        best_target_q = tf.reduce_max(target_q, axis=1)
        Q_sample = self.r + mask * self.config.gamma * best_target_q
        action = tf.one_hot(self.a, num_actions)
        Q_new = tf.reduce_sum(action * q, axis=1)
        self.loss = tf.reduce_mean(tf.squared_difference(Q_sample, Q_new))

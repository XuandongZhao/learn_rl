...
class BaselineNetwork(object):
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.scope = scope
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder,1,self.scope,self.config.n_layers,self.config.layer_size,output_activation=self.config.activation), 1)
    self.loss = tf.losses.mean_squared_error(self.baseline_target_placeholder, self.baseline)
    self.update_baseline_op = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss)
    #######################################################
    #########          END YOUR CODE.          ############

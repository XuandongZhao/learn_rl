...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      self.sampled_action = action_means + tf.multiply(log_std.exp(), tf.random_normal(self.batch_size))
      self.lobprob = tf.log(tf.contrib.distributions.MultivariateNormalDiag())
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      for t in range(rewards.shape[0] - 1, -1, -1):
        current_return = rewards[t] + self.config.gamma * previous_return

...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for s in range(nS):
			v = value_function[s]
			a = policy[s]
			value_function[s] = sum([prob * (r + gamma * value_function[next_s]) for (prob, next_s, r, _) in P[s][a]])
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for s in range(nS):
		q_vals = np.zeros(nA)
		for a in range(nA):
			q_vals[a]= sum([prob * (r + gamma * value_from_policy[next_s]) for (prob, next_s, r, _) in P[s][a]])
